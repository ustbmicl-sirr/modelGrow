...
class BasicBlock(nn.Module):
    ...
    def __init__(self, planes, stride=1):
        ...
        self.feature1 = nn.Conv2d(planes, planes, (3, 3), stride, (1, 1))
        self.feature2 = nn.Conv2d(planes, planes, (3, 3), 1, (1, 1))
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, planes, stride=1):
        ...
        self.feature2 = nn.Conv2d(planes//4, planes//4, (3, 3), stride, (1, 1))
        self.feature3 = nn.Conv2d(planes//4, planes, kernel_size=1, bias=False)

    def forward(self, x):
        out = F.relu(self.feature1(x))
        out = F.relu(self.feature2(out))
        out = F.relu(self.feature3(out) + x)
        return out
...
class ResNet4Block(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, image_channels=3):
        ...
        self.downstream1 = self._make_downstream(in_dim=64, out_dim=96)
        self.downstream2 = self._make_downstream(in_dim=96, out_dim=128)
        self.downstream3 = self._make_downstream(in_dim=128, out_dim=256)
    ...
    def _make_downstream(self, in_dim, out_dim):
        return nn.Conv2d(in_channels=in_dim, out_channels=out_dim, kernel_size=(3, 3), stride=2, padding=1, bias=False)
...
def get_runtime_basic_resnet(depths, num_classes=10, image_channels=3):
    if len(depths) == 3:
...
def get_runtime_bottleneck_resnet(depths, num_classes=10, image_channels=3):
    if len(depths) == 3:
...
if __name__ == "__main__":
    import torch
    model = get_runtime_bottleneck_resnet([1,2,3,4])
