...
class MobileNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, image_channels=3):
        ...
        self.layer2 = self._make_layer(block, num_blocks[1], kernel_size=3, size=32)
        self.layer3 = self._make_layer(block, num_blocks[2], kernel_size=5, size=64)
        self.layer4 = self._make_layer(block, num_blocks[3], kernel_size=5, size=128)
        self.layer5 = self._make_layer(block, num_blocks[3], kernel_size=5, size=128)
        self.downstream1 = self._make_downstream(in_dim=16, out_dim=32)
        self.downstream2 = self._make_downstream(in_dim=32, out_dim=64)
        self.downstream3 = self._make_downstream(in_dim=64, out_dim=128)
        self.downstream4 = self._make_downstream(in_dim=128, out_dim=128)
...
def get_small_mobilenetv3(num_classes=10, image_channels=3):
    return MobileNet(MobileNetBlock, [1, 1, 2, 2, 2], num_classes=num_classes, image_channels=image_channels)


def get_large_mobilenetv3(num_classes=10, image_channels=3):
    return MobileNet(MobileNetBlock, [2, 2, 3, 6, 2], num_classes=num_classes, image_channels=image_channels)
